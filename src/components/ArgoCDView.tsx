    ...
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-6 animate-fade-in">
      ...
      {!argoEnabled ? (
        <div className="bg-[#0E1117] border border-[#1C212B] rounded-2xl p-12 text-center flex flex-col items-center justify-center gap-3">
          ...
          <button
            onClick={onToggleArgo}
            className="mt-2 px-4 py-2 bg-[#67df70] hover:bg-[#52c95b] text-[#0A0C10] rounded-xl font-mono text-xs font-semibold shadow-sm transition-colors cursor-pointer"
          >
            Enable ArgoCD Sync
          </button>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div className="bg-[#0E1117] border border-[#1C212B] rounded-xl p-3.5 flex flex-col">
              ...
              <span className={`font-mono text-[10px] mt-1 ${outOfSyncCount > 0 ? 'text-amber-400 font-semibold' : 'text-[#64748B]'}`}>
            ...
            <div className="bg-[#0E1117] border border-[#1C212B] rounded-xl p-3.5 flex flex-col">
              ...
              <span className="font-mono text-[10px] text-[#64748B] mt-1">
                {healthyCount} / {applications.length} applications healthy
              </span>
            </div>
                  ...
                  >
                    <span
                      className={`material-symbols-outlined text-[15px] ${
                        isRefreshing ? 'animate-spin text-[#67df70]' : ''
                      }`}
                    >
                      refresh
                    </span>
            ...
            <div className="px-5 py-2 bg-[#0E1117] border-b border-[#1C212B] flex items-center gap-2 overflow-x-auto text-xs font-mono">
              <button
                ...
                className={`px-2.5 py-1 rounded-md transition-colors cursor-pointer ${
                  statusFilter === 'all'
                    ? 'bg-[#1C212B] text-white font-semibold'
            ...
            {filteredApps.length === 0 ? (
              <div className="p-8 text-center flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs text-[#94A3B8] font-mono">
                  No applications matched the filter &quot;{searchQuery}&quot;
                </p>
                {searchQuery && (
                  <button
                    onClick={() => setSearchQuery('')}
              ...
              <div className="divide-y divide-[#1C212B]">
                {filteredApps.map((app) => {
                  ...
                  const linkedService = services.find(
                    (s) => s.id === app.serviceId || s.argoAppName === app.name
                  );
